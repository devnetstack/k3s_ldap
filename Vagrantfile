...
Vagrant.configure(2) do |config|
  config.vm.define 'master' do |master|
    ...
    master.vm.provision "shell", inline: <<-SHELL
      ...
      curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--node-ip=${IPADDR} --flannel-iface=enp0s8 --write-kubeconfig-mode 644 --kube-apiserver-arg="service-node-port-range=30000-30100" --kube-apiserver-arg="authentication-token-webhook-config-file=/vagrant/authn/authn_config.yaml" --no-deploy=servicelb --no-deploy=traefik" K3S_STORAGE_BACKEND=etcd3 K3S_STORAGE_ENDPOINT="http://127.0.0.1:2379" sh -
  ...
  config.vm.define 'ldap' do |ldap|
    ldap.vm.box = default_box
    ldap.vm.hostname = "ldap"
    ldap.vm.synced_folder ".", "/vagrant", type:"virtualbox"
    ldap.vm.network 'private_network', ip: "192.168.0.254",  virtualbox__intnet: true
    ldap.vm.network "forwarded_port", guest: 22, host: 2222, id: "ssh", disabled: true
    ldap.vm.network "forwarded_port", guest: 22, host: 2254
    ...
    ldap.vm.provision "shell", inline: <<-SHELL
      ...
      sudo systemctl reload ssh
    SHELL
    ldap.vm.provision "shell", path: "ldap/build_ldap.sh"
    ldap.vm.provision "shell", path: "authn/build_authn.sh"
  end
